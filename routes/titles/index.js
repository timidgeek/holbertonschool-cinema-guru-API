...
router.get('/', verifyToken, async (req, res) => {
    Title.findAll().then(titles => res.send(titles))
})
...
router.get('/advancedsearch', verifyToken, async (req, res) => {
    ...
    const favorites = await UserFavourites.findAll({ where: { UserId: req.userId }, attributes: ["TitleId"] })
    const watchLater = await UserWatchLater.findAll({ where: { UserId: req.userId }, attributes: ["TitleId"] })
    res.send({ totalCount: titles.length, titles, favorites, watchLater })
})
